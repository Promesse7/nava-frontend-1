...
const ProfilePage = () => {
  ...
  const SettingItem = ({ icon: Icon, title, onClick, isLogout = false }) => (
    <button
      ...
      className={`flex items-center justify-between w-full py-3 border-b ${
        isLogout
          ? "border-none text-red-600 hover:text-red-700"
    ...
    >
      ...
      <ChevronRight
        className={`w-5 h-5 ${isLogout ? "text-red-500" : "text-gray-500"}`}
      />
    </button>
  );
  ...
  if (loading) {
    return (
      <p className="text-center mt-6 font-[Orbitron] justify-center">
        Loading profile...
      </p>
    );
  }
  ...
  return (
    <div className=" mx-auto w-[80%] p-6 bg-white rounded-lg shadow-lg">
      ...
      {/* Header Section */}
      <div className="flex items-center  w-full max-w-screen-lg px-2 mb-10 justify-around">
        {/* Back Button */}
        <button
          onClick={() => navigate(-1)}
      ...
      <form onSubmit={handleUpdate} className="space-y-6">
        ...
        <div className="container mx-auto px-4 mt-6 flex justify-center ">
          <button
            ...
            className="bg-black hover:bg-grey-700 text-white-500 text-white px-4 py-2 rounded"
          >
            Save Changes
          </button>
        </div>
